...
```bash
$ composer require josesierradev/laravel-quickbooks-client
```

2. Run our migration to install the `quickbooks_tokens` table:

```bash
$ php artisan migrate --package=josesierradev/laravel-quickbooks-client
```
...
1. You will need a ```quickBooksToken``` relationship on your ```User``` model.  There is a trait named ```JoseSierraDev\QuickBooks\HasQuickBooksToken```, which you can include on your ```User``` model, which will setup the relationship. To do this implement the following:

Add ```use JoseSierraDev\QuickBooks\HasQuickBooksToken;``` to your service container at the top of User.php
...
>>> $quickbooks = app('JoseSierraDev\QuickBooks\Client') // or app('QuickBooks')
=> JoseSierraDev\QuickBooks\Client {#1613}
...
If you have routes that will be dependent on the user's account having a usable QuickBooks OAuth token, there is an included middleware ```JoseSierraDev\QuickBooks\Laravel\Filter``` that gets registered as ```quickbooks``` that will ensure the account is linked and redirect them to the `connect` route if needed.
